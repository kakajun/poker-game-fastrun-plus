...
class ActionSpace:
    ...
    def _get_key(self, play: Play) -> str:
        """生成唯一键: Type_Len_Max_Kicker"""
        rank_val = play.max_rank
        if hasattr(rank_val, 'value'):
            rank_val = rank_val.value

        key = f"{play.type.name}_{play.length}_{rank_val}"
        if play.type == HandType.TRIPLE_WITH_SINGLE and play.kicker_rank > 0:
            kicker_val = play.kicker_rank
            if hasattr(kicker_val, 'value'):
                kicker_val = kicker_val.value
            key += f"_{kicker_val}"
        return key
    ...
    def build(self):
        ...
        # Triple+Single (带单张，自动选择最小的)
        for r in range(3, 16):
            self._add_action_abstract(HandType.TRIPLE_WITH_SINGLE, 1, r)

        # Triple+Single (带特定单牌)
        for triple_r in range(3, 16):
            for single_r in range(3, 16):
                if triple_r != single_r:
                    self._add_action_abstract(HandType.TRIPLE_WITH_SINGLE, 1, triple_r, kicker_rank=single_r)
    ...
    def _add_action_abstract(self, type: HandType, length: int, max_rank: int, kicker_rank: int = 0):
        key = f"{type.name}_{length}_{max_rank}"
        if type == HandType.TRIPLE_WITH_SINGLE and kicker_rank > 0:
            key += f"_{kicker_rank}"

        if key not in self.action_to_id:
            idx = len(self.action_to_id)
            self.action_to_id[key] = idx
            self.id_to_action[idx] = Play(type, [], length, max_rank, kicker_rank=kicker_rank)
